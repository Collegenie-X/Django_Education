...
class UserAdmin(BaseUserAdmin):
    fieldsets = (
        ...
        (_('Email verification'), {'fields': ('email_verification_code', 'is_email_verified')}),
    ...
    list_display = ('email', 'username',  'is_email_verified','is_active','last_login', 'start_date')
    list_filter = (
        'is_active',
        'is_email_verified',
    ...
    def toggle_email_verification(self, request, queryset):
        ...
        for user in queryset:
            ...
            user.save()
            updated_count += 1
        self.message_user(request, f"{updated_count} 사용자의 이메일 인증 상태가 변경되었습니다.")
    ...
    def delete_model(self, request, obj):
        try:
            ...
            self.message_user(request, f"사용자 {obj.email} 삭제 중 오류 발생: {str(e)}", level="error")
    ...
    def delete_queryset(self, request, queryset):
        try:
            ...
            self.message_user(request, f"사용자 삭제 중 오류 발생: {str(e)}", level="error")
