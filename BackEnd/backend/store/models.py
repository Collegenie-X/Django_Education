...
class UnitType (models.Model):
    ...
    subject = models.CharField(max_length=50, help_text="* required",choices=SUBJECT_CHOICES,null=True,blank=True)
...
class PreviewImage(models.Model):
    problem = models.ForeignKey('Problem', on_delete=models.CASCADE, related_name='preview_images')
    ...
    def clean(self):
        ...
        if self.image and self.image_url:
            raise ValidationError('이미지를 업로드하거나 이미지 URL 중 하나만 선택해야 합니다.')
        if self.image:
            ext = os.path.splitext(self.image.name)[1].lower()
            if ext not in ['.jpg', '.jpeg', '.png', '.svg']:
                raise ValidationError('지원되지 않는 파일 형식입니다. jpg, jpeg, png, svg만 가능합니다.')
        if self.image_url:
...
class Problem(models.Model):
    ...
    subject = models.CharField(max_length=50, help_text="* required",choices=SUBJECT_CHOICES, db_index=True)
    problem_type = models.CharField(max_length=20,choices=PROBLEM_TYPE, blank=True, null=True, default="Normal") 
    type = MultiSelectField(choices=TYPE_CHOICES, default="Short Answer", help_text="* select one or more types")
    grade = models.CharField(max_length=100, choices=GRADE_CHOICES, blank=True, null=True, default=None)
    
    unit = models.ManyToManyField(UnitType, related_name="problems", blank=True)
    detailed_section = models.ManyToManyField(SectionType, related_name="DetailSection",blank=True)
    
    difficulty = models.IntegerField(choices=DIFFICULTY_CHOICES, default=1, 
                                     help_text="* 상 중 하 등급으로 구분됩니다.")
    
    title = models.CharField(max_length=100, db_index=True, help_text="* 100글자 이내 작성해 주세요.")
    description = models.TextField(db_index=True,help_text="* 300글자 이내로 작성해 주세요.")
    
    discounted_price = models.DecimalField(max_digits=7, decimal_places=2,blank=True,null=True,default=0, 
                                help_text="*달러 기준이며, $1 ~ $100 사이에서 입력해야 합니다.")
    price = models.DecimalField(max_digits=7, decimal_places=2, default=5.00, 
                                help_text="*달러 기준이며, $1 ~ $100 사이에서 입력해야 합니다.")
    is_free = models.BooleanField(default=False, help_text="공짜일 경우에는 선택해 주세요.") 
    is_view =   models.BooleanField(default=True, help_text="비공개일때는 체크를 제거해 주세요.", db_index=True)
    ...
    file = models.FileField(
        ...
        # storage=file_storage,  # 로컬 파일 시스템 사용
        storage=AttachmentS3Boto3Storage(),  ### S3 샤용
        help_text="* required",
        null=True, 
        blank=True,
        default=""
    ...
    def clean(self):
        ...
        if self.discounted_price is not None:
            if not (0 <= self.discounted_price < self.price):
                raise ValidationError('Discounted price must be between $1 and less than the original price.')
    ...
    def total_reviews_score(self):
        ...
        total_score = reviews.aggregate(models.Sum('rating'))['rating__sum']
        
        # 평균값 계산
        if total_score and reviews.count() > 0:
...
class Wishlist(models.Model):
    ...
    problem = models.ForeignKey('Problem', on_delete=models.CASCADE, related_name='wishlisted_by')
