...
class UnitTypeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'subject', 'problem_subject')
        }),
    )
...
class SectionTypeAdmin(admin.ModelAdmin):
    list_display = ['id', 'name', 'subject','problem_subject', 'problem_units','problem_title']
    list_filter = ['subject']
    search_fields = ['name']
    ordering = ['name', 'subject']
    ...
    # 문제와 연결된 subject들을 반환하는 메서드
    def problem_subject(self, obj):
        related_problems = obj.DetailSection.all()  # Many-to-Many 관계로 연결된 Problem 객체
        subjects = set(problem.subject for problem in related_problems)  # 중복 제거
        return ", ".join(subjects) if subjects else "No related subjects"
    ...
    def problem_units(self, obj):
        related_problems = obj.DetailSection.all()  # Many-to-Many 관계로 연결된 Problem 객체
        units = set(unit.name for problem in related_problems for unit in problem.unit.all())  # 중복 제거
    ...
    def update_section_subject_by_problems(self, request, queryset):
        ...
        for section in queryset:
            related_problems = section.DetailSection.all()  # Many-to-Many 관계로 연결된 Problem 객체
            if related_problems.exists():
                section.subject = related_problems.first().subject  # 첫 번째 문제의 subject로 설정
                section.save()
                updated_count += 1
            else:
                skipped_count += 1
        ...
        for section in queryset:
            related_problems = section.DetailSection.all()  # Many-to-Many 관계로 연결된 Problem 객체
            units = set(unit for problem in related_problems for unit in problem.unit.all())  # 중복 제거
            if units:
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'subject','problem_subject', 'problem_units')
        }),
    )

    readonly_fields = ['problem_subject', 'problem_units']  # 문제 subjects 및 units 필드는 읽기 전용으로 설정

admin.site.register(SectionType, SectionTypeAdmin)

class PreviewImageInline(admin.TabularInline):
    model = PreviewImage
    extra = 1  # 기본으로 표시할 폼 개수
    ...
    def image_tag(self, obj):
        ...
        if obj.image and hasattr(obj.image, 'url'):
            ...
            img_url = "https://studyola-main-dir.s3.us-east-2.amazonaws.com/" + obj.image_url if obj.image_url else None
...
class ProblemAdmin(admin.ModelAdmin):
    ...
    formfield_overrides = {
        models.ManyToManyField: {'widget': SelectMultiple(attrs={'size': '15'})},  # 높이를 15로 설정
    }
        ...
        if images:
            ...
            for image in images:
                ...
                if image.image and hasattr(image.image, 'url'):
                    ...
                    # image 필드가 없을 때 image_url 사용
                    img_url =  "https://studyola-main-dir.s3.us-east-2.amazonaws.com/" + image.image_url

                # 이미지 URL이 있는 경우 태그 생성
                if img_url:
                    image_tags.append(f'<img src="{img_url}" width="60" height="80" style="margin-right:5px;" />')
                else:
    ...
    def display_units(self, obj):        
        # 알파벳 순으로 정렬하여 반환
        return ", ".join([f"{unit.name} ({unit.subject})" for unit in obj.unit.order_by('name')])

    def display_detailed_sections(self, obj):
        # 알파벳 순으로 정렬하여 반환
        return ", ".join([f"{section.name} ({section.subject})" for section in obj.detailed_section.order_by('name')])
    ...
    fieldsets = (
        ...
        ('Subject', {
            'fields': ('subject', 'type', 'grade')
        }),
        ('Category', {
            'fields': ('unit', 'detailed_section')
        }),
        ('Difficulty', {
            'fields': ('difficulty',)
        }),
        ('Contents', { 'fields':  ('title', 'description')}),        
        ('Etc(PDF)', { 'fields':  ('price', 'discounted_price', 'is_free', 'pages', 'problems') }),
        ('Files', {
            'fields': ('file',),
        })         
    )
    ...
    list_filter = [
        'is_view','grade', 'subject', 'problem_type', 'price','discounted_price', 
        'type', 'unit', 'detailed_section', 'difficulty', 'pages', 'problems'
    ]
    ordering = ['-created_date', '-updated_date']

    # Mapping of subjects to S3 key paths
    subject_to_image_key = {
        "English": "pre_image/2024/09/27/english_title.png",
        "Liberal Arts": "pre_image/2024/09/27/LiberalArts_title.png",
        "Math": "pre_image/2024/09/27/Math_title.png",
        "Science": "pre_image/2024/09/27/Science_title.png",
        "STEM": "pre_image/2024/09/27/Stem_title.png",
        "Puzzle": "pre_image/2024/10/04/puzzle_title.png",
        "SAT-Math": "pre_image/2024/09/30/SAT_Math_title.png",
        "SAT-English": "pre_image/2024/09/30/SAT_English_title.png",
        
        "Multiple Choice": "pre_image/2024/10/08/Multiple_Choice.png",
        "Short Answer": "pre_image/2024/10/08/Short_Answer.png",
        "Mathematical Modeling": "pre_image/2024/10/08/Mathematical_Modeling.png",
        "Learning Through Discussion": "pre_image/2024/10/08/Learnin__Through_Discussion.png",
        "STEAM": "pre_image/2024/10/08/STEAM.png",
        "Math Puzzle": "pre_image/2024/10/08/Math_Puzzle.png",
        "Math for Literacy": "pre_image/2024/10/08/Math_for_Literacy.png"
    ...
    def make_best(self, request, queryset):
        ...
        self.message_user(request, f"{updated} problem(s) were successfully marked as Best.", messages.SUCCESS)

    @admin.action(description='Mark selected problems as Premium')
    def make_premium(self, request, queryset):
        updated = queryset.update(problem_type='Premium')
        self.message_user(request, f"{updated} problem(s) were successfully marked as Premium.", messages.SUCCESS)
    ...
    def make_normal(self, request, queryset):
        ...
        self.message_user(request, f"{updated} problem(s) were successfully marked as Normal.", messages.SUCCESS)


    @admin.action(description='Update selected problems with subject-based preview images (Math)')
    def update_preview_images_by_subject(self, request, queryset):
        updated_count = 0
        skipped_count = 0
        ...
        for problem in queryset:
            ...
            if problem.subject == "Math":
                if problem.type in [
                    ...
                    "Learning Through Discussion", "STEAM", "Math Puzzle", "Math for Literacy"
                ]:
                    image_key = self.subject_to_image_key.get(problem.type)
                else:
                    image_key = None
            ...
            if image_key:
                # 기존의 모든 PreviewImage 삭제
                problem.preview_images.all().delete()
                # 새 PreviewImage 생성
                PreviewImage.objects.create(problem=problem, image=image_key)
                updated_count += 1
            else:
                skipped_count += 1
                self.message_user(
                    request, 
                    f"문제 '{problem}'은(는) 주제 '{problem.subject}'에 해당하는 이미지 키가 없어 건너뛰었습니다.", 
                    messages.WARNING
                )
        
        message = f"{updated_count}개의 문제가 주제 기반의 프리뷰 이미지로 업데이트되었습니다."
        if skipped_count > 0:
            message += f" {skipped_count}개의 문제는 이미지 키가 없어 업데이트되지 않았습니다."
    ...
    def delete_preview_images(self, request, queryset):
        ...
        self.message_user(request, f"{deleted_count} preview images deleted.", messages.SUCCESS)
    ...
    def toggle_is_free(self, request, queryset):
        ...
        self.message_user(request, f"{updated_count} problem(s) had their Free/Paid status toggled.", messages.SUCCESS)
    ...
    def toggle_is_view(self, request, queryset):
        ...
        self.message_user(request, f"{updated_count} problem(s) had their visibility toggled.", messages.SUCCESS)

    actions = [
        make_best, 
        make_premium, 
        make_normal, 
        update_preview_images_by_subject, 
